...
assert.equal(rules.requirementScore({ chairs: 6, tables: 2, arch: true, audio: true }), 10);
assert.equal(rules.requirementScore({ chairs: 2, tables: 1, arch: false, audio: false }), 3);
...
assert.equal(rules.gradeJob({ readiness: 10, verified: true, cueScore: 3, detours: 0, overloads: 0 }).rank, "S");
