...
The prototype includes generated interaction/cue sounds, pause and mute controls, clear-skies and wind-advisory contracts, an optional five-minute relaxed shift, solid collision, local career records, pure-rule tests, and no build step. Known boundaries: keyboard-first controls, no networked persistence, no multiplayer, lightweight authored physics, and one venue with two contract conditions. Those omissions are prioritized in `docs/BUILD_PREP.md` rather than hidden as implied functionality.
